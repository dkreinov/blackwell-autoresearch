...
// v1: fp16 HuberLoss (smooth_l1). pred,target (32768,32768) fp16. Output: scalar.
// smooth_l1(d) = 0.5*d^2 if |d|<1, else |d|-0.5
// Single pass, float4 (8 halfs), 2048 blocks x 1024t. float32 accumulation.

__global__ void huber_fp16_v1(
    ...
    for (; j + 7 * stride < n4; j += stride8) {
        ...
        #define HUB8(P, T) {                                                       \
            ...
            s += SL1(p0.x-t0.x)+SL1(p0.y-t0.y)+SL1(p1.x-t1.x)+SL1(p1.y-t1.y)  \
               + SL1(p2.x-t2.x)+SL1(p2.y-t2.y)+SL1(p3.x-t3.x)+SL1(p3.y-t3.y); }

        HUB8(P0,T0); HUB8(P1,T1); HUB8(P2,T2); HUB8(P3,T3);
        HUB8(P4,T4); HUB8(P5,T5); HUB8(P6,T6); HUB8(P7,T7);
        #undef HUB8
    }
    for (; j < n4; j += stride) {
...
torch::Tensor huber_fp16_cuda(torch::Tensor pred, torch::Tensor target) {
    ...
    huber_fp16_v1<<<2048, 1024>>>(pp, tp, sum_t.data_ptr<float>(), n4);
    return (sum_t / (float)n).to(torch::kHalf).squeeze();
}
"""

cpp_source = "torch::Tensor huber_fp16_cuda(torch::Tensor pred, torch::Tensor target);"

module = load_inline(
    name='huberloss_fp16_v1',
    cpp_sources=cpp_source,
    cuda_sources=cuda_source,
    functions=['huber_fp16_cuda'],
...
class ModelNew(nn.Module):
    ...
    def forward(self, predictions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        return module.huber_fp16_cuda(predictions.contiguous(), targets.contiguous())
